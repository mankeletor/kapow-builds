...
make install DESTDIR=$MEOWCOIN_ROOT/tmp-destdir
...
tar cf $PKG.tar ./
echo $PASS | sudo -S alien $PKG.tar $PKG.deb --description="Meowcoin is an experimental \
digital currency that enables instant payments to anyone, anywhere in the world. \
The Meowcoin platform also lets anyone create assets (tokens) on the Meowcoin network. \
Assets can be used for NFTs, STOs, Gift Cards, and fractional ownership of anything of \
value. Meowcoin uses peer-to-peer technology to operate with no central authority: \
