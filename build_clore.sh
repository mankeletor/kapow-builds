...
make install DESTDIR=$CLORE_ROOT/tmp-destdir
...
tar cf $PKG.tar ./
echo $PASS | sudo -S alien $PKG.tar $PKG.deb --description="Clore Core is the original Clore \
client and it builds the backbone of the network. It downloads and, by default, stores the \
entire history of Clore transactions; depending on the speed of your computer \
and network connection, the synchronization process is typically complete \
in under an hour. \
\
To download compiled binaries of the Clore Core and wallet, \
visit the [GitHub release page](https://gitlab.com/cloreai-public/blockchain/releases). \
