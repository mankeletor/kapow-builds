...
make install DESTDIR=$PAPRIKA_ROOT/tmp-destdir
echo -e "${amarillo}=======Building package=======${fin_color}"
cd $PAPRIKA_ROOT/tmp-destdir
mkdir -p ./usr/share/applications
mkdir -p ./usr/share/icons
cp ../doc/paprikacoin_logo_doxygen.png ./usr/share/icons/paprikacoin.png
echo '
...
tar cf $PKG.tar ./
echo $PASS | sudo -S alien $PKG.tar $PKG.deb --description="Paprikacoin is an experimental \
digital currency that enables instant payments to anyone, anywhere in the world. The Paprikacoin \
platform also lets anyone create assets (tokens) on the Paprikacoin network. Assets can be used for \
NFTs, STOs, Gift Cards, and fractional ownership of anything of value.\
Paprikacoin uses peer-to-peer technology to operate with no central authority: managing transactions \
